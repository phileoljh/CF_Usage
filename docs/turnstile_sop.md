...
在開始配置前，請先瞭解 Cloudflare Turnstile 的核心特性：

- **無需 Cloudflare 託管網域**：Turnstile 是一個獨立服務。不論您的網域是否交由 Cloudflare 代理 (Proxied) 或託管，皆可直接使用。
- **無驗證次數限制 (Unlimited)**：官方免費版不限驗證次數，適合各種規模的流量，不用擔心爆量或產生額外費用。
- **Hostname 授權機制**：
  - 為了安全，必須在控制台明確指定授權的 Hostnames。
  - 設定根網域 (如 `example.com`) 會自動授權其所有子網域。
  - **本地開發**：若需在本地測試，請務必手動將 `localhost` 加入授權清單，或使用測試專用的 Dummy Keys。
- **數據保留**：
  - 免費版提供 **7 天** 的分析報告回顧功能。
  - 敏感數據 (如 IP、指紋) 通常保留不超過數週以維護隱私。
- **儀表板顯示說明**：
  - 本專案儀表板中顯示的 `10,000 Req` 額度為**視覺占位符**（僅用於呈現百分比進度條），並非實際的官方限制。
