...
3. 若需暫時關閉此功能，可在環境變數中新增 `ENABLE_TURNSTILE` 並設為 `false`，即可預設放行。
...
### 附註：Cloudflare GraphQL D1 資料集差異 (開發筆記)
在客製化或除錯 D1 監控數據時，請務必使用正確的 GraphQL 資料集，以免取得無效或錯誤的數值：

*   **✅ `d1AnalyticsAdaptiveGroups` (本專案使用)**
    *   **用途**：專門用於「高階聚合統計」與「計費額度計算」。
    *   **特性**：此端點能正確回傳總讀取行數 (`rowsRead`) 與總寫入行數 (`rowsWritten`)，數據行為與 Cloudflare 官方儀表板一致，適用於總量監控。
*   **❌ `d1QueriesAdaptiveGroups` (不適用於總量統計)**
    *   **用途**：專門用於「微觀效能除錯」。
    *   **特性**：此端點會記錄具體的 SQL 查詢語法、執行毫秒數等細節。因其專注於特定查詢的效能分析，若用來計算全域總量，容易發生資料漏算或回傳 Null 的問題。

*(註：Cloudflare GraphQL 分析資料管線通常有 5~15 分鐘的彙整延遲，此為官方架構特性，若部署後數值未立即跳動請稍候片刻。)*
